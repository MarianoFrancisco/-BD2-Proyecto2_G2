...
export class UpdateInfoComponent implements OnInit, OnDestroy {
  ...
  public user?: User;
  public email: FormControl = this.formBuilder.control('', [Validators.required, Validators.minLength(5), Validators.pattern(emailPattern)]);
  ...
  public onUpdatePwd(): void {
    if (this.updatePwd.passwordForm.valid) {
      ...
      this.authService.updatePwd(valueForm.contrasenia_actual, valueForm.contrasenia).subscribe({
